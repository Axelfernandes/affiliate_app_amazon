      ...
      {isLoading && suggestions.length === 0 ? (
        ...
        <div className="p-20 text-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
          ...
          <p className="text-slate-400/60 mt-2">
            No new trends captured yet. Use the Scout to find deals.
          </p>
        </div>
      ) : (
              ...
              <div className="flex-1 space-y-2">
                ...
                {(s.category || s.store) && (
                  <div className="flex gap-2 pt-1">
                    {s.store && s.store !== "Other" && (
                      <span className="bg-indigo-50 text-indigo-600 text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md border border-indigo-100">
                        {s.store}
                      </span>
                    )}
                    {s.category && s.category !== "Other" && (
                      <span className="bg-slate-100 text-slate-500 text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md border border-slate-200">
                        {s.category}
                      </span>
                    )}
                  </div>
