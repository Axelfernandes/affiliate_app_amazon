    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        Papa.parse(file, {
            ...
            complete: async (results: Papa.ParseResult<any>) => {
                ...
                if (!firstRow || (!firstRow.name && !firstRow.productName) || (!firstRow.url && !firstRow.sourceUrl)) {
                    setError('Invalid CSV format. Please ensure you have "name" and "url" columns.');
                ...
                for (const row of data) {
                    try {
                        ...
                        const reason = row.reason || row.reasonForSuggestion || 'Imported via bulk upload';
            ...
            <div className="p-8 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50 text-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="mt-4 flex flex-col items-center gap-2">
                        ...
                        <p className="text-xs text-gray-400">Required columns: <code className="bg-gray-100 px-1 rounded">name</code>, <code className="bg-gray-100 px-1 rounded">url</code></p>
                    </div>
                </div>
            </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            <div>
                                ...
                                <div className="mt-1 space-y-1">
                                    <p className="text-sm text-green-700">Successfully added <span className="font-bold">{result.success}</span> products to your inbox.</p>
                                    {result.failed > 0 && (
                                        <p className="text-sm text-amber-700">Skipped <span className="font-bold">{result.failed}</span> rows due to errors or missing data.</p>
                                    )}
                                </div>
                            </div>
                        </div>
                        <button onClick={() => setResult(null)} className="text-green-600 hover:bg-green-100 p-1 rounded-lg">
                            <X size={20} />
                        </button>
                    </div>
                </div>
            ...
            <div className="p-6 bg-white border border-gray-100 rounded-2xl shadow-sm">
                ...
                <div className="bg-gray-900 rounded-xl p-4 overflow-x-auto">
                    <pre className="text-blue-400 text-xs font-mono">
                        {`name,url,reason
"Logitech MX Master 3S","https://amazon.com/dp/B09HM94V6G","Best office mouse 2024"
"Sony WH-1000XM5","https://amazon.com/dp/B09XS7JWHH","Top rated noise cancelling"
"Apple 2024 MacBook Air","https://amazon.com/dp/B0CX2257N1","M3 chip productivity"
`}
                    </pre>
                </div>
                <p className="text-sm text-gray-500 mt-3 italic">Tip: You can use "productName" or "name", and "sourceUrl" or "url" as headers.</p>
            </div>
        </div>
    );
}
